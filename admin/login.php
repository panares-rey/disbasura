...
$db = get_db();
$no_admin = !$db->query("SELECT id FROM users WHERE role='admin' LIMIT 1")->fetch();
$error = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($no_admin && isset($_POST['full_name'])) {
        ...
        if ($fn && $un && $em && $pw) {
            $db->prepare("INSERT INTO users (full_name,username,email,password,role) VALUES (?,?,?,?,?)")
               ->execute([$fn,$un,$em,password_hash($pw,PASSWORD_BCRYPT),'admin']);
            header('Location: /disbasura/admin/login.php'); exit;
        }
        $error = 'All fields required.';
    } else {
        $u = $db->prepare("SELECT * FROM users WHERE username=? AND role='admin'");
